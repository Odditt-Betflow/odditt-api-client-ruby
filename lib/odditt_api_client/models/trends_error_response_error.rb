...
module OddittApiClient
  class TrendsErrorResponseError < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OddittApiClient::TrendsErrorResponseError` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OddittApiClient::TrendsErrorResponseError`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      ...
      code_validator = EnumAttributeValidator.new('String', ["BAD_REQUEST", "VALIDATION_ERROR", "AMBIGUOUS_LOOKUP", "UNAUTHORIZED", "FORBIDDEN", "NOT_FOUND", "RATE_LIMIT_EXCEEDED", "INTERNAL_ERROR"])
    ...
    def code=(code)
      validator = EnumAttributeValidator.new('String', ["BAD_REQUEST", "VALIDATION_ERROR", "AMBIGUOUS_LOOKUP", "UNAUTHORIZED", "FORBIDDEN", "NOT_FOUND", "RATE_LIMIT_EXCEEDED", "INTERNAL_ERROR"])
